...
// 末尾スラッシュ正規化。Netlify は prerender 出力（/route/index.html＝ディレクトリ型）を
// 末尾スラッシュで配信し、/route → /route/ へ 301 する。よって発行URL（sitemap/feed/llms/canonical）も
// 末尾スラッシュに揃え、「sitemap のURLが毎回301」状態を解消して新ドメインのインデックスを綺麗にする。
// ルート '/'・ファイル（.xml 等）・クエリ/フラグメント付きは対象外。
function withSlash(path) {
  if (path === '/' || path.includes('?') || path.includes('#') || /\.[a-z0-9]+$/i.test(path)) return path;
  return path.replace(/\/+$/, '') + '/';
}
...
function renderSitemap(siteUrl, routes) {
  const urls = routes
    .map((path) => `  <url><loc>${escapeXml(siteUrl + withSlash(path))}</loc></url>`)
    .join('\n');
...
function renderLlmsTxt(siteUrl, posts, genres) {
  ...
  const genreLines = genres
    .map((g) => `- [${g.name}](${u('/discover/' + g.id)}): ${g.tagline}`)
    .join('\n');
  return `# サブスクやめた
...
- アフィリエイトリンクには（PR）表記がある。詳細: [収益開示](${u('/disclosure')})
- 運営者情報: [このサイトについて](${u('/about')})

## 主要ツール

- [解約手順トップ](${u('/')}): ${SERVICES.length}サービスの解約ページ直リンクと3ステップ手順
- [解約方法の一覧・比較](${u('/compare')}): ${SERVICES.length}サービスの解約難易度・月額目安・注意点・解約ページ直リンクを1ページで一覧・比較（ランキングなし・両論併記）
- [固定費の棚卸し](${u('/tracker')}): 契約中サブスクの月額・年額合計と見直し順を可視化（登録不要）
- [サブスク図鑑](${u('/discover')}): ジャンル別に乗り換え先を特徴・弱点つきで比較
- [やめて買う](${u('/yamete-kau')}): 月額をやめて買い切りで済ませる代替案
- [お役立ち記事](${u('/blog')}): 解約・乗り換え・買い切り化の解説記事（RSS: ${siteUrl}/feed.xml）
