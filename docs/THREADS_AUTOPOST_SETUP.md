...
1. スマホの Threads アプリ（または threads.net）で **「サブスクやめた」名義の新規アカウント**を作成。
   - Instagram と紐づく仕様。サブスクやめた用の Instagram を作ってから連携するのが管理上きれい（既存の個人IGと混ぜない）。
2. プロフィールに 1行説明＋ `https://sabusuku.netlify.app` を入れる。
3. これで投稿先が存在する状態になる。
