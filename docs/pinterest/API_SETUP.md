...
## ✅ 進捗（2026-07-14）：Standardアクセスの審査を提出済み・結果待ち

俊雄さんがデモ動画（Chrome操作をClaudeが実演・Win+Alt+Rで録画→ffmpegで前後カット）を撮影し、フォーム項目（アプリ名・ビジネス名・URL是正・使用目的「ピン作成・予約投稿」のみ・オーディエンス「ビジネス」のみ）を入力の上、reCAPTCHA・送信まで完了。
`/apps/1582125/` で確認すると「リクエストされたアクセス：アクセス『Standard』へのアップグレードは保留中です」と表示され、審査キューに入ったことを確認済み。**現在のアクセスレベルはまだ「Trial」のまま**（審査完了まで変わらない・下記の制限は解消されるまで有効）。
審査が下りたら「3. 投稿する」に進める。数日〜のリードタイムが見込まれるため、次にこのファイルを開いたら、まずここを再確認すること。
