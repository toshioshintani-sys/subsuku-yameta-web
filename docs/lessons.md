...
- 未着手の関連課題：`google-one`のPLANSにある古い「AIプレミアム」項目（2TB表記）と新設`gemini-advanced`（5TB表記）の整合性確認・要すり合わせ。→ **2026-07-14 対応済み（下記参照）**

---

## 2026-07-14 ★★ google-one PLANSの古い「AIプレミアム」項目を削除（gemini-advanced新設との整合性確保）

### 発見
- 前日（2026-07-13）に別立てした`gemini-advanced`（Google AI Pro・5TB・月2,900円）と、`google-one`のPLANSに残っていた旧項目「AI プレミアム（2TB + Gemini Advanced）月2,900円」が、同じ価格で容量表記だけ矛盾していた（2TB vs 5TB）。
- 対処：`google-one`のPLANSから該当行を削除（ベーシック100GB／プレミアム2TBの純粋ストレージ2プランのみに整理）。`howToCheck`に「Gemini込みの上位プランは『Google AI Pro』を参照」を追記し、AI込みプランを探すユーザーが迷わないよう導線だけ残した。EXTENDED_CONTENTの既存darkPatterns（「AI機能つき上位プランへの誘導」への対処文言）はそのまま整合しているため変更不要と判断。
- ローカルに`git worktree`でorigin/main（PR#24マージ後）を別途チェックアウトして確認・修正した：作業中の別ブランチ（`claude/price-change-history`）に2026-07-08頃のPinterest関連の未コミット差分が残っていたため、それを壊さず・触らずに済ませる目的。

### なぜ重要か
1. **「新規サービスを別立てする」判断をした時点で、旧サービス側に残る重複項目の除去までをワンセットで完了させないと、翌日以降に矛盾データとして観測者（ユーザー）の目に触れ続ける。** 07-13時点で「スコープ外・別タスク」として記録を残す判断自体は正しかったが、次に触る時は即座に拾って閉じること。
2. **セッション間でブランチが分岐・停滞していると、どのブランチで作業すべきか自明ではない。** `git log --graph --all`とorigin/mainの実HEADを確認しないまま作業すると、存在しないコード（未マージのgemini-advanced）を編集しようとして空振りする。作業前に対象ブランチの実際の位置関係を確認する習慣が必要。
3. **未コミットの変更が残ったブランチに気づいたら、`git worktree add`で別ディレクトリに退避して作業すると、無関係な既存差分を壊さずに済む。** stashより安全（同一ブランチの巻き戻りリスクがゼロ）。

### 永続化
- `src/data/services.js`（`fix/google-one-ai-plan-cleanup`ブランチ）
- ローカル`claude/price-change-history`ブランチに残る2026-07-08 Pinterest関連の未コミット差分は本タスクでは未着手のまま（別セッションの持ち物の可能性があり、無断で削除しなかった）。次にそのブランチで作業する時に要確認。
