...
## 8. コピペ用（第1バッチ・差し込み済み）
> 各動画＝そのまま貼れる4ブロック。①生成プロンプトをGemini等へ → ②③④をYouTube投稿画面へ。差し込み済みなので編集不要。
...
```
縦9:16・約30秒の解説ショート動画を作って。日本語。トーンは誠実・淡々・親切（煽らない）。
[0.0-2.0s] フック全画面テロップ：「Amazonプライム、“継続する”の大きなボタンに惑わされないで」
[2.0s-] 手順を1ステップ1カット・大きな白文字テロップ＋簡単なアイコン：
  STEP1「アカウント＆リスト →『プライム会員情報』」
  STEP2「『プライム会員資格を終了する』をタップ」
  STEP3「『特典を終了する』→『特典を終了する』で確定」
[終盤] 注意テロップ：「⚠ 『継続する』が目立つ作り。解約は小さいリンクを探す」
[最後2s] CTA：「全サブスクの解約手順 → サブスクやめた」（URLは概要欄）
ナレーションはテロップと同内容を読み上げ。BGMは控えめ。右下に小さく「サブスクやめた」。
実在のロゴ・著作物は使わない（一般的なUIの簡易図解で表現）。
```
**タイトル**
```
【惑わされない】Amazonプライムの解約方法｜“継続する”の罠
```
**概要欄**
```
Amazonプライムの解約を最短で終わらせる手順です。

▼ 全サブスクの解約手順・無料代替・乗り換え先はこちら（煽らず両論併記）
https://sabusuku-yameta.com/service/amazon-prime?utm_source=youtube&utm_medium=social&utm_campaign=shorts&utm_content=amazon-prime
...
**タグ**
```
サブスク, 解約, 解約方法, Amazonプライム 解約, 節約, 固定費
```
...
```
縦9:16・約30秒の解説ショート動画を作って。日本語。トーンは誠実・淡々・親切（煽らない）。
[0.0-2.0s] フック全画面テロップ：「U-NEXTの解約リンク、わざとページ最下部に隠されてます」
[2.0s-] 手順を1ステップ1カット・大きな白文字テロップ＋簡単なアイコン：
  STEP1「マイメニュー →『契約内容の確認・変更』」
  STEP2「『解約はこちら』（ページ下部の小さいリンク）」
  STEP3「理由を選択 →『解約する』」
[終盤] 注意テロップ：「⚠ 解約リンクがとても見つかりにくい。ページ最下部を探して」
[最後2s] CTA：「全サブスクの解約手順 → サブスクやめた」（URLは概要欄）
ナレーションはテロップと同内容を読み上げ。BGMは控えめ。右下に小さく「サブスクやめた」。
実在のロゴ・著作物は使わない（一般的なUIの簡易図解で表現）。
```
**タイトル**
```
【最短】U-NEXTの解約方法｜隠れた解約リンクの場所
```
**概要欄**
```
U-NEXTの解約を最短で終わらせる手順です。

▼ 全サブスクの解約手順・無料代替・乗り換え先はこちら（煽らず両論併記）
https://sabusuku-yameta.com/service/u-next?utm_source=youtube&utm_medium=social&utm_campaign=shorts&utm_content=u-next
...
```
縦9:16・約30秒の解説ショート動画を作って。日本語。トーンは誠実・淡々・親切（煽らない）。
[0.0-2.0s] フック全画面テロップ：「Spotify、アプリからは解約できません。理由はこれ」
[2.0s-] 手順を1ステップ1カット・大きな白文字テロップ＋簡単なアイコン：
  STEP1「ブラウザで account の subscription ページを開く（アプリ不可）」
  STEP2「『プランを変更する』→『Premiumをキャンセルする』」
  STEP3「手順に沿って完了」
[終盤] 注意テロップ：「⚠ アプリ内課金だとアプリから解約不可。必ずブラウザで」
[最後2s] CTA：「全サブスクの解約手順 → サブスクやめた」（URLは概要欄）
ナレーションはテロップと同内容を読み上げ。BGMは控えめ。右下に小さく「サブスクやめた」。
実在のロゴ・著作物は使わない（一般的なUIの簡易図解で表現）。
```
**タイトル**
```
【なぜ？】Spotifyがアプリで解約できない理由と最短手順
```
**概要欄**
```
Spotifyの解約を最短で終わらせる手順です（アプリからは解約できません）。

▼ 全サブスクの解約手順・無料代替・乗り換え先はこちら（煽らず両論併記）
https://sabusuku-yameta.com/service/spotify?utm_source=youtube&utm_medium=social&utm_campaign=shorts&utm_content=spotify
