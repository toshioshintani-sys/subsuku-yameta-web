...
export const PLANS = {
  ...
  'line-music': {
    plans: [
      ...
      // 2026-08-19 ファミリープラン（1,480円・最大6人）を削除した。同ページの現行プラン表は
      // 一般と学生しか載せていない。ファミリーの語はページ内に4回出るが4回とも古い宣伝文の中で、
      // 同じ文が一般980円・学生480円という改定前の価格を併記している（そこにある1,680円も現行値ではない）。
      // 掲載していた1,480円は公式のどこにも存在せず、7/25時点で既に未確認と注記されていた。
      // 現在も提供されているか確認できないプランは、価格を推測して出すより出さないほうが安全。
 ...
 *  2026-09-01: TTS 160.80 / TTB 158.80 → TTM 159.70
 *  ⚠️ 同じページに「外貨現金両替相場」が並んでいる。**別物**。
 *     必ず上段の対顧客電信相場（US Dollar / 米ドル / USD の行）の TTS・TTB を使うこと。 */
export const USD_JPY = 159.70;
/** ページに書かれていた「相場日」。土日に読むと最終営業日になる */
export const USD_JPY_AS_OF = '2026-09-01';
/** 実際にページを読んだ日。AS_OF と違えば「最終営業日の値」という注釈が出る */
export const USD_JPY_READ_ON = '2026-09-02';
