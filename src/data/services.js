...
export const EXTENDED_CONTENT = {
  ...
  'xbox-game-pass': {
    ...
    whyHard:
      '手続きは Microsoft アカウントのサブスク管理から数クリック。ただし「自動更新」が既定オンで放置課金になりやすく、Ultimate へ体験加入した後の通常課金も見落としやすい。解約とは残り期間を使い切れる「自動請求の無効化」。',
    darkPatterns: [
  ...
  'nintendo-switch-online': {
    ...
    faq: [
      ...
      { q: '本体から解約できますか？', a: 'Switch 本体からも状況は確認できますが、ニンテンドーアカウントのページ（ブラウザ/スマホ）からの手続きが確実です。' },
    ],
  },
  'apple-tv-plus': {
    summary:
  ...
  'amazon-music-unlimited': {
    ...
    whyHard:
      '解約手順は数クリックだが、「Prime Music（プライム特典）」と「Music Unlimited（別料金）」の違いが分かりにくく、どれを解約するのか迷いやすい。Unlimited の解約は会員情報ページから「サブスクリプションのキャンセル」で行う。',
    darkPatterns: [
  ...
  dropbox: {
    ...
    whyHard:
      '解約はアカウントの「プランの管理」から数クリックと簡単。ただし解約すると無料プラン（少容量）に戻るため、保存量が無料枠を超えていると同期が止まる・一部ファイルを扱いにくくなる点に注意。年間契約の途中解約は次回更新の停止になる。',
    darkPatterns: [
  ...
  'canva-pro': {
    ...
    afterCancel:
      '解約しても支払い済み期間の終了まで Pro 機能を利用できる。終了後は無料版に戻り、Pro 限定素材・機能が使えなくなる。作成済みデザインは残るが、Pro 素材を含む部分は再編集・書き出しに制限が出ることがある。必要なデザインは事前に高解像度で書き出しておくと安心。',
    faq: [
  ...
  'rakuten-tv': {
    ...
    whyHard:
      '「解約」の対象が定額パックごとに分かれており、マイページの「ご契約情報」からどのパックを契約しているか確認して個別に解約する必要がある。レンタル/購入は都度課金なので解約という概念がなく、混同しやすい。',
    darkPatterns: [
    ...
    afterCancel:
      '定額パックを解約しても、購入済み・レンタル期間中の作品は期限まで視聴できる。楽天会員・楽天ポイントには影響しない。パ・リーグ Special などシーズン型は解約タイミングで損得が変わるため、更新日を確認してから手続きするとよい。',
    faq: [
      { q: '購入した作品は解約後も見られますか？', a: 'はい。「購入」した作品は定額パック解約後も視聴できます。ただしサービス全体の提供終了時には視聴できなくなるリスクがある点は他の配信サービスと同様です。' },
  ...
  'wowow-on-demand': {
    ...
    whyHard:
      '難易度は高め。解約ページまでのログイン → アンケート入力 → 完了と段階が多く、月末締めの契約サイクルのため手続きのタイミングで課金が1ヶ月延びることがある。放送（BS）契約とオンデマンドの関係も分かりにくい。',
    darkPatterns: [
    ...
    faq: [
      ...
      { q: 'いつまでに手続きすれば今月末で解約できますか？', a: '月末までに解約手続きを完了すれば、その月での解約になるのが基本です。月をまたぐと翌月分が発生するため、余裕を持って月末数日前までに済ませるのが安全です。' },
  ...
  fod: {
    ...
    faq: [
      { q: '自分がどこ経由で契約したか分かりません', a: 'クレジットカードや携帯料金の明細で請求元を確認するのが確実です。「APPLE」「GOOGLE」ならアプリ内課金、「AMAZON」ならAmazon経由、それ以外はFOD直契約の可能性が高いです。' },
      { q: '解約しても無料分は見られますか？', a: 'はい。プレミアム解約後も、無料の見逃し配信は視聴できます。' },
      { q: '月の途中で解約したら日割りになりますか？', a: '日割り返金は基本ありません。更新日を確認し、期間を使い切ってから解約するのが損のないタイミングです。' },
    ],
  },
  lemino: {
    summary:
  ...
  'dmm-premium': {
    ...
    whyHard:
      '解約は専用ページにログインして「解約する」を押すだけと短いが、解約ページへの入口がサービス内から見つけにくく、直接URLにたどり着けるかが実質の難所。DMMポイント払い・クレカ払いなど支払い方法で更新日の考え方が異なる。',
    darkPatterns: [
    ...
    faq: [
      ...
      { q: '無料体験中に解約するとすぐ見られなくなりますか？', a: '体験期間の扱いは時期のキャンペーンによって異なりますが、多くの場合は期間末まで利用できます。解約手続きだけ先に済ませておくのが安全です。' },
      { q: '解約とアカウント退会は違いますか？', a: '違います。月額のプレミアム解約だけならアカウントも購入物も残ります。退会（アカウント削除）は購入済みコンテンツも失うので、固定費を止める目的なら解約だけにしてください。' },
    ],
  },
  'niconico-premium': {
    summary:
      'ニコニコ動画プレミアムは、動画の画質・シーク・生放送の優先入場などが強化される月額会員。ニコニコ文化圏のヘビーユーザー向けで、視聴が減ると恩恵を感じにくくなる典型的な「惰性継続」サブスク。',
    whyHard:
    ...
    faq: [
      ...
      { q: '「退会」と書いてあるのが不安です', a: 'ニコニコの表記では「プレミアム退会」＝月額課金の解約で、アカウント削除ではありません。安心して進めてください。' },
    ],
  },
  crunchyroll: {
    summary:
    ...
    whyHard:
      '解約自体はアカウントの Membership ページから「Cancel Membership」を押すだけと簡単な部類。ただし画面が英語基調で、アプリストア経由の契約だと解約場所が変わる点は他サービスと同様の注意が要る。',
    darkPatterns: [
  ...
  'rakuten-music': {
    summary:
      '楽天ミュージックは楽天の音楽配信。楽天ポイントが貯まる・使える点と、聴く時間が短い人向けのライトプランがあるのが特徴。楽天カード/楽天モバイルユーザー向けの割引もある。',
    whyHard:
      '解約はWebのマイページ →「ご利用プラン」→「解約する」で完了する簡単な部類。ただしアプリ内課金（App Store/Google Play）契約はストア側での解約になる点と、楽天の他サービス同様にキャンペーン経由の契約だと更新日が分かりにくい点に注意。',
    darkPatterns: [
    ...
    afterCancel:
      '解約後は期間末までフル再生でき、その後は再生不可になる。プレイリストや楽曲のお気に入りはアカウントに残る。楽天会員・楽天ポイントには影響しない。',
    faq: [
  ...
  'soundcloud-go': {
    ...
    faq: [
      ...
      { q: 'アプリから解約できますか？', a: 'アプリ内課金で契約した場合は各ストアのサブスクリプション管理から解約します。Web契約はブラウザの設定ページから行います。' },
  ...
  dmagazine: {
    ...
    whyHard:
      '解約はdマガジン公式の解約ページ → dアカウントでログイン →「手続きを完了する」の流れ。dアカウントの認証が挟まるためパスワード忘れで中断しやすく、最後の完了ボタンまで進みきらないと解約されていないことがある。',
    darkPatterns: [
    ...
    faq: [
      ...
      { q: '月の途中で解約すると損ですか？', a: '日割り返金はないため、更新日直前の解約が最も無駄がありません。ただし忘れるリスクを考えると、思い立った時に解約する方が結果的に得なことが多いです。' },
    ],
  },
  bookwalker: {
    summary:
    ...
    whyHard:
      '読み放題など月額サービスの解約は設定から可能だが、サイトの「退会」ページはアカウント自体の削除で、**退会すると購入済みの書籍がすべて読めなくなる**。固定費を止めたいだけの人が誤って退会すると被害が大きい。',
    darkPatterns: [
  ...
  honto: {
    ...
    whyHard:
      '退会ページから手続き自体は短いが、**退会すると購入済みの電子書籍が読めなくなり、共通ポイントや書店との連携も解除される**。影響範囲が電子書籍だけでなくリアル書店のポイントにも及ぶ点が見落としやすい。',
    darkPatterns: [
      { trigger: '退会の影響範囲（電子書籍＋ポイント＋店舗連携）が一覧で見えにくい', response: '退会前に「読みかけの電子書籍」「ポイント残高」「店舗カード連携」の3点を確認する' },
  ...
  'rakuten-kobo': {
    ...
    darkPatterns: [
      ...
      { trigger: '退会の巻き添え範囲（ポイント・市場・カード）が見えにくい', response: '退会確認前に楽天ポイント残高・楽天カード・他の楽天サービス利用状況を必ず棚卸しする' },
    ],
    afterCancel:
  ...
  evernote: {
    ...
    darkPatterns: [
      ...
      { trigger: '解約時の割引オファー', response: '使っていないなら割引でも固定費。まず更新を止め、必要ならエクスポートしてから移行先を試す' },
    ],
    afterCancel:
    ...
    faq: [
      ...
      { q: '年間プランの途中解約は返金されますか？', a: '原則、日割り返金はなく次回更新が止まる形です。更新日を確認して、期間を使い切ってからの解約が無駄のないタイミングです。' },
    ],
  },
  'apple-one': {
    summary:
    ...
    whyHard:
      '解約自体は iPhone の「設定」→ Apple ID →「サブスクリプション」→ Apple One から数タップ。難しいのは解約後の設計で、Apple One をやめると含まれていたサービスが一括で止まるため、残したいもの（例：iCloud+ の容量）だけ個別contract し直す判断が要る。',
    darkPatterns: [
    ...
    faq: [
      { q: 'iCloudの写真は消えますか？', a: 'すぐには消えませんが、無料5GBを超えた状態では新しいバックアップや同期が止まります。写真が多い場合は iCloud+ を単体契約で残すのが現実的です。' },
  ...
  'google-one': {
    ...
    whyHard:
      '解約は one.google.com の設定から「メンバーシップをキャンセル」で数クリック。難所は解約後の容量超過で、無料15GBを超えたままだと新しいメールの受信やファイル保存に支障が出るため、先にデータ整理か移行の算段を立ててからの解約が安全。',
    darkPatterns: [
    ...
    faq: [
      { q: '解約したら写真やメールは消えますか？', a: 'すぐには消えません。ただし無料枠を超えたままだと新規保存・受信が制限されます。長期間超過が続いた場合の削除ポリシーもあるため、早めの整理が安全です。' },
  ...
  'icloud-plus': {
    ...
    faq: [
      { q: '写真が5GBを超えています。解約したら消えますか？', a: 'すぐには消えませんが、新しい写真の同期が止まります。パソコンや外部ストレージ、他のクラウドへ写真を退避してからダウングレードするのが安全です。' },
  ...
  '1password': {
    ...
    afterCancel:
      '解約後は読み取り専用モードになり、既存のパスワードの閲覧とエクスポートはできるが、新規追加・編集ができなくなる。移行先を決めてエクスポート（CSV等）→ インポート → 動作確認、まで済ませてからの解約が安全。家族プランは共有メンバー全員に影響する。',
    faq: [
  ...
  figma: {
    ...
    faq: [
      ...
      { q: '席数を減らすだけでも安くなりますか？', a: 'なります。編集者（有料席）を閲覧者に変更すれば席数分の課金が減ります。全員に編集権限が必要なチームは少ないので、まず権限の棚卸しがおすすめです。' },
    ],
  },
  'deepl-pro': {
    summary:
    ...
    faq: [
      ...
      { q: '仕事でたまに長文を訳します。Proは必要？', a: '頻度次第です。月に数回なら無料版＋分割で足りることが多く、毎日大量に訳す・機密文書を扱うなら Pro の価値があります。' },
    ],
  },
  pairs: {
    summary:
    ...
    whyHard:
      '難易度は高い。アプリ内に解約ボタンがなく、App Store/Google Play 課金ならストアのサブスクリプション管理、クレカ（Web）決済ならブラウザ版のマイページと、契約経路によって場所が全く違う。さらに「有料プランの解約」と「アカウント退会」は別で、退会だけしても課金が止まらない事故が起きやすい。',
    darkPatterns: [
    ...
    faq: [
      ...
      { q: '退会と解約はどちらが先ですか？', a: '有料プランの解約が先です。退会だけしても課金契約が残る場合があります。解約 → 期間終了の確認 → 退会、の順が安全です。' },
  ...
  'github-copilot': {
    ...
    whyHard:
      '個人契約は GitHub の設定ページから「Cancel Copilot」を2回確認で完了と簡単。ただし会社（Organization）契約の場合は管理者経由でしか外せない。また2026年4月以降、個人向け新規サインアップが停止されており、一度解約すると再契約できない場合がある点が独特の注意点。',
    darkPatterns: [
  ...
  'vimeo-pro': {
    ...
    whyHard:
      '設定の「Billing」→「Cancel plan」で手続き自体は短い。難所は解約後の保存数制限で、無料プランの上限を超えたアップロード済み動画の扱いを決めてからでないと、公開中の埋め込み動画が視聴できなくなるリスクがある。',
    darkPatterns: [
    ...
    faq: [
      { q: 'アップロードした動画は消えますか？', a: '即削除ではありませんが、無料枠を超えた分は制限対象になります。原本を手元に残し、公開が必要な動画は代替先へ移してから解約するのが安全です。' },
  ...
  match: {
    ...
    afterCancel:
      '自動更新を停止しても支払い済み期間の末日までは有料機能を使える。完全にやめる場合は、期間満了後にアカウント削除を別途行う。解約確認メールは保管しておくと請求トラブル時に役立つ。',
    faq: [
  ...
  'discord-nitro': {
    ...
    whyHard:
      'ユーザー設定 →「サブスクリプション」→「Nitroをキャンセル」で完了と簡単な部類。注意点はサーバーブーストで、Nitro 特典のブーストを友人のサーバーに付けている場合、解約でブーストが外れてサーバーのレベルが下がる。',
    darkPatterns: [
  ...
  'linkedin-premium': {
    summary:
      'LinkedIn Premium はビジネスSNS LinkedIn の有料プラン。転職活動向け（Career）や営業向け（Sales Navigator）などがあり、無料トライアルから自動課金される典型パターンと、月額の高さで見直し対象になりやすい。',
    whyHard:
    ...
    afterCancel:
      '解約後は請求期間の末日まで Premium 機能（閲覧者の確認・InMail など）が使え、その後無料アカウントに戻る。プロフィール・つながり・投稿には影響しない。転職活動の山場だけ契約する「必要な月だけ」運用が現実的。',
    faq: [
      { q: '解約するとプロフィールや人脈は消えますか？', a: '消えません。無料アカウントとしてすべて残ります。Premium 限定機能が使えなくなるだけです。' },
      { q: '無料トライアルだけで終わらせるには？', a: '開始直後に解約手続きを済ませるのが確実です。解約してもトライアル期間の最終日までは Premium 機能を使えます。' },
